      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <div key={index} className={`bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow ${plan.popular ? 'ring-4 ring-red-600 scale-105' : ''}`}>
              {plan.popular && (
              ...
              <div className="text-4xl font-bold text-red-600 mb-4">{plan.price}<span className="text-lg text-gray-600">/mês</span></div>
              ...
              <button className={`w-full py-3 rounded-lg font-semibold transition-colors ${plan.popular ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>
                Contratar Plano
              </button>
            </div>
          ))}
